...
function manIskinStart(): string {
  return [
    ...
    block(
      "ОПИСАНИЕ",
      "Сначала имитация взлома в терминале (команды и логи), затем сброс экрана, полноэкранная загрузка «Прямой канал», потом ASCII-баннер Искина до judge. Требуется cat revelation.txt."
    ),
    LINE,
  ].join("\n");
}
