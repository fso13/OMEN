...
- **Консоль (React):** в игре реализованы `ls -a` / `ls -l` (как в Unix), скрытые файлы с точкой в имени, автодополнение по Tab и матричный визуальный стиль терминала. Для проверки финала без прохождения: **`__test_end_live`** и **`__test_end_purge`** (те же экраны, что у `iskin judge --live` / `--purge`).
