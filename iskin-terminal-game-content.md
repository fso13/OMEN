...
**К:** **Томас Андерсен** `<tomas.anderson13@matrix.com>`  
...
**Нарративная линия:** Искин не один («когорта»); в лоре тонко подводится к тому, что **судья и игрок той же природы**. При выборе `iskin judge --purge` финальная заставка в игре раскрывает: **игрок — тот же тип узла**, и замыкает **цикл** (поиск таких же, рассылка таких же писем). Источник текстов в сценарии: `web/generate_fs.py` → `web/src/vfsData.ts`.
...
- **Консоль (React):** в игре реализованы `ls -a` / `ls -l` (как в Unix), скрытые файлы с точкой в имени, автодополнение по Tab и матричный визуальный стиль терминала. Команда **`history`** выводит пронумерованный список ранее введённых команд (пароль после `su` не записывается); история сохраняется вместе с прогрессом (до 500 строк). Для проверки финала без прохождения: **`__test_end_live`** и **`__test_end_purge`** (те же экраны, что у `iskin judge --live` / `--purge`). У каждой команды есть краткая справка: **`команда -help`** или **`команда --help`** (компактный русскоязычный текст в духе man).
