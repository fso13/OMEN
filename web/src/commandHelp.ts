...
function manCat(): string {
  return [
    ...
    block("СИНТАКСИС", "cat ФАЙЛ [ПАРОЛЬ]"),
    block(
      "ОПИСАНИЕ",
      "Путь может быть относительным к текущему каталогу. Цепочка в Documents/puzzles/ (файлы 02–05): без пароля из предыдущего слоя доступ закрыт; после расшифровки: cat ИМЯ ПАРОЛЬ."
    ),
    LINE,
  ].join("\n");
}
